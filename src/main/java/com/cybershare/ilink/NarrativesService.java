...
public class NarrativesService 
{
			...
			if(variable.get(structureName).getAsString().replaceAll("\"", "")
				...
				if(!isScenario) {
					for(int j = 0; j < values.size(); j++) {
						...
						}else if(varValueName == null || varValueName.equals("")) {
							//Get values
							varValue.put(String.valueOf(0),
									Float.valueOf("582.86")); //historical value, todo: retrieve from swim avginflows
	...
	private void openConnection() {
		if(testing)
		{
			...
			MongoClientURI mongoUri = new MongoClientURI("mongodb://ilinkadmin:%40test3ilink@129.108.18.45:27017");
			mClient = new MongoClient(mongoUri);
		}
		mMorphia = new Morphia();
		mDataStore = mMorphia.createDatastore(mClient, "watermodels");
	...
	private void createDummyTemplate() {
		...
		listTemplatesAvgInflows.add(buildTemplate("swim-terms:Template",
				"Water Supply outcome causes a ~adjective_trend ~adjective_behaviour in average inflows to Elephant Butte in comparison to the historical period (1995-2015). Historical averages were 582.86 KAF/yr and the selected scenario results in ~average ~element_unit average annual inflow.",Arrays.asList("all","other"),
						Arrays.asList("urban"),
						Arrays.asList("El Paso"),"en"));
		listTemplatesAvgInflows.add(buildTemplate("swim-terms:Template",
				"El valor de el ingreso de agua cause un ~adjective_trend ~adjective_behaviour en promedio a Elephant Butte en comparasión con el periodo histórico (1995-2015). El promedio histórico es de 582.86 KAF/yr y el escenario configurado resulta en un promedio anual de ingreso de ~average ~element_unit",
				Arrays.asList("all","other"),
						Arrays.asList("urban"),
						Arrays.asList("El Paso"),"es"));
